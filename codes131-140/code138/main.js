...
function reverseWords(s) {
    let removeEmptySpaces = s.trim().split(" ")
    for (let i=0; i<removeEmptySpaces.length; i++) {
        if (removeEmptySpaces[i]==="") {
            removeEmptySpaces.splice(i, 1)
            i--
        }
    }
    return removeEmptySpaces.reverse().join(' ')
}
...
//explanation tweet: https://twitter.com/Codingwithroy/status/1674821402799529987?s=20
...
function sortByBit(arr) {
    return arr.sort((a, b)=> {
        let aBits = a.toString(2).split('').filter(bit => bit==="1").length
        let bBits = b.toString(2).split('').filter(bit => bit==="1").length
        if(aBits === bBits) {
            return a-b
        } else {
            return aBits - bBits
...
//explanation tweet: https://twitter.com/Codingwithroy/status/1663609713748062209?s=20